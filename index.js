...
bot.onText(/\/info/, async (msg) => {
  ...
  if (userId == OWNER_ID) {
    ...
    sisa = "∞";
  } else if (db[userId] && db[userId].until > Date.now()) {
    const sisaHari = Math.ceil((db[userId].until - Date.now()) / (1000 * 60 * 60 * 24));
...
bot.on('message', async (msg) => {
  ...
  if (!pendingData[inviter].groups) pendingData[inviter].groups = [];

  // Tambahkan grup baru ke daftar milik user
  if (!pendingData[inviter].groups.includes(chatId)) {
    pendingData[inviter].groups.push(chatId);
  ...
  try {
    ...
    // Jika grup kurang dari 15 member, keluar otomatis
    if (memberCount < 15) {
      await bot.leaveChat(chatId);
      await bot.sendMessage(
        inviter,
        `🚫 Grup "${ctx.chat.title}" tidak valid karena hanya memiliki ${memberCount} member.\nMinimal harus 15 member agar dianggap valid.`
      );
      return; // hentikan proses
    }
  } catch (err) {
    console.log(`Gagal cek member grup ${chatId}:`, err.message);
  }

  // Hitung jumlah grup valid milik user
  let validCount = 0;
  for (const gid of pendingData[inviter].groups) {
    try {
...
bot.getMe().then(async (botInfo) => {
  ...
  const text = `✅ *Bot Berhasil Aktif!*\n\n🤖 *Nama Bot:* @${botInfo.username}\n🆔 *Bot ID:* \`${botInfo.id}\`\n⏰ *Waktu:* ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`;

  try {
    await bot.sendMessage(OWNER_ID, text, { parse_mode: "Markdown" });
    console.log("📩 Notifikasi dikirim ke owner.");
  } catch (err) {
    console.error("⚠️ Gagal kirim pesan ke owner:", err.message);
  }
});